...
route.post("/", async (req: CustomRequest, res: Response, next: NextFunction) => {
  ...
  try {
    const imagePromises = images.filter((i) => i.length > 0).map((image) => doWithRetries(async () => fetch(image)));

    const imageResponses = await Promise.all(imagePromises);

    const bufferPromises = imageResponses.map((imageResponse) =>
      doWithRetries(async () => Buffer.from(await imageResponse.arrayBuffer()))
    );
